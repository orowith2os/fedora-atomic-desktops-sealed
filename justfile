...
variants := '{
  ...
  "kinoite": {
    "image_url": "quay.io/fedora-ostree-desktops/kinoite",
    "fedora_release_and_tag": [[44, 44], [45, 45]]
  },
  "bootc-44": {
  ...
  "bootc-rawhide": {
    "image_url": "quay.io/bootc-devel/fedora-bootc-rawhide-standard",
...
# Internal recipe, to get the image url from a variant
_get_image_url variant:
    #!/usr/bin/env bash
    set -euo pipefail
...
# Internal recipe, to get the image tag from a variant and fedora release
_get_image_tag variant fedora_release:
    #!/usr/bin/env bash
    set -euo pipefail
    ...
    image_tag=$(
        jq -r \
           --arg v "{{variant}}" \
           --arg r "{{fedora_release}}" \
           '
           first(
               .[$v].fedora_release_and_tag[]
               | select((.[0] | tostring) == ($r | tostring))
           )[1]
           ' <<< '{{variants}}'
    )
...
build variant fedora_release:
    #!/bin/bash
    set -euo pipefail

    image_url=$(just _get_image_url {{variant}})
    image_tag=$(just _get_image_tag {{variant}} {{fedora_release}})
...
build-base variant fedora_release:
    #!/bin/bash
    set -euo pipefail
...
libvirt variant fedora_release:
    ...
    # OS variant being fedora43 is fine here; to quote Timothée:
    # This was hardcoded because this will fail with numbers that are for Rawhide / branched releases (and there is no difference).
